  ...
  <el-card class="ev-multiples-chart" shadow="never" :body-style="{ padding: '4px 4px 8px' }">
    <template #header>
      <div class="ev-multiples-chart__header">
        <StockCardTitle title="現金獲利估值倍數" :info-text="INFO_TEXT" />
    ...
    <template v-else-if="gaugeStatsByMetric.evEbitda && gaugeStatsByMetric.evToEbit">
      ...
      >
        <SharedChart v-loading="history.pending.value" class="ev-multiples-chart__chart" :option="option" :init-options="{ renderer: 'svg' }" autoresize />
